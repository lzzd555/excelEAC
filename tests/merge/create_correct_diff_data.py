...
df_a.to_excel('tests/merge/sample_data/table_a_correct.xlsx', index=False)
print("✅ 创建表A数据（正确映射测试）: table_a_correct.xlsx")

# 创建表B数据：使用列名 员工编号, 部门编码, 岗位, 薪资, 入职年份
table_b_data = {
    '员工编号': ['001', '002', '003', '005'],  # 004在表B中不存在
    '部门编码': ['销售部', '销售部', '市场部', '技术部'],  # 直接与表A的部门值匹配
    '岗位': ['经理', '专员', '主管', '架构师'],
    '薪资': [15000, 8000, 12000, 20000],
    '入职年份': [2020, 2020, 2020, 2021]
}
df_b = pd.DataFrame(table_b_data)
df_b.to_excel('tests/merge/sample_data/table_b_correct.xlsx', index=False)
print("✅ 创建表B数据（正确映射测试）: table_b_correct.xlsx")
